...
module "firewall" {
  ...
  authorized_ssh_ranges = ["0.0.0.0/0"]
...
module "gke" {
  ...
  enable_private_endpoint = false # Public endpoint for dev access
  master_ipv4_cidr_block  = "172.16.0.0/28"
  master_authorized_networks = [
    {
      cidr_block   = "0.0.0.0/0" # Allow from anywhere in dev
  ...
  use_preemptible_nodes = true # Use preemptible for cost savings
